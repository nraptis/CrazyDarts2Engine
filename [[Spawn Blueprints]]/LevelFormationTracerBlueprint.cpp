...
int LevelFormationTracerBlueprint::CountObjectsOfType(int pGameObjectType) {
    int aResult = 0;
    for (int i=0;i<mSpawnNodeList.mCount;i++) {
        LevelFormationNodeBlueprint *aNode = (LevelFormationNodeBlueprint *)mSpawnNodeList.mData[i];
        if (aNode->mObjectType == pGameObjectType) {
            aResult += 1;
        }
    }
    return aResult;
...
void LevelFormationTracerBlueprint::Build(LevelFormationTracer *pTracer) {
    ...
    pTracer->mSpecialWidth = (float)mSpecialWidth;
    pTracer->mSpecialHeight = (float)mSpecialHeight;
...
FJSONNode *LevelFormationTracerBlueprint::Save() {
    ...
    if (mSpecialType == TRACER_SPECIAL_TYPE_ROUNDED_RECT) {
        aExport->AddDictionaryInt("special_type_width", mSpecialWidth);
        aExport->AddDictionaryInt("special_type_height", mSpecialHeight);
...
void LevelFormationTracerBlueprint::Load(FJSONNode *pNode) {
    ...
    if (mSpecialType == TRACER_SPECIAL_TYPE_CIRCLE) {
        mSpecialRadius = pNode->GetInt("special_type_radius", 256);
    }
    
    if (mSpecialType == TRACER_SPECIAL_TYPE_ROUNDED_RECT) {
        mSpecialCornerRadius = pNode->GetInt("special_type_corner_radius", 60);
        mSpecialWidth = pNode->GetInt("special_type_width", 256);
        mSpecialHeight = pNode->GetInt("special_type_height", 256);
