...
int LevelSectionPermBlueprint::CountObjectsOfType(int pGameObjectType) {
    int aResult = 0;
    if (IsPathPerm()) {
        for (int i=0;i<mSpawnCount;i++) {
...
void LevelSectionPermBlueprint::DeleteSpawnFromIndex(int pIndex) {
    if (pIndex >= (mSpawnCount)) { return; }
    if (pIndex < 0) { return; }
    ...
    if (mSelectedSpawnIndex >= mSpawnCount) {
        mSelectedSpawnIndex = mSpawnCount - 1;
    }
    mSpawnCount--;
}
...
void LevelSectionPermBlueprint::Build(LevelSectionPerm *pPerm) {
    ...
    pPerm->mShortCircuit = mShortCircuit;
    pPerm->mShortCircuitExternal = mShortCircuitExternal;
    
    
    pPerm->mSpawnEqualSpacing = mSpawnEqualSpacing;
    pPerm->mSpawnSpacing = mSpawnSpacing;
    pPerm->mSpawnOffset = mSpawnOffset;
...
FJSONNode *LevelSectionPermBlueprint::Save() {
    ...
    if (mSpawnOffset != 0) { aExport->AddDictionaryInt("spawn_offset", mSpawnOffset); }
    ...
    if (IsPathPerm()) {
        ...
        if ((mGrid != NULL) && (mGrid->mGridEnabled == true)) {
            aExport->AddDictionary("grid", mGrid->SaveCurrentGrid());
        }
...
void LevelSectionPermBlueprint::Load(FJSONNode *pNode) {
    ...
    mSpawnOffset = pNode->GetInt("spawn_offset", 0);
    ...
    FJSONNode *aGridNode = pNode->GetDictionary("grid");
    
    if (aGridNode != NULL) {
        if (mGrid == NULL) {
            mGrid = new GameEditorGrid();
        }
        mGrid->LoadCurrentGrid(aGridNode);
    }
